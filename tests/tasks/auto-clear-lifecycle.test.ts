...
import { projectSlugDir, sessionFilePath } from "./helpers/task-store-paths.ts";
...
const sessionFile = (sessionId: string) => sessionFilePath(agentDir, cwd, sessionId);
...
async function runAndFinish(mock: Mock, ctx: any, count: number): Promise<void> {
  ...
  for (let i = 0; i < count; i++) {
    const res = await mock.executeTool(
      "TaskCreate",
      { subject: `Task ${i}`, description: "d" },
      ctx,
    );
...
describe("auto-clear across batches", () => {
  ...
  it("starts the next batch clean instead of appending to the finished one", async () => {
    ...
    await mock.fireLifecycle("turn_start", {}, ctx);
    const created = await mock.executeTool(
      "TaskCreate",
      { subject: "Fresh work", description: "d" },
      ctx,
    );
  ...
  it("keeps every step of a list the agent builds one task at a time", async () => {
    ...
    for (const [i, subject] of ["Step one", "Step two", "Step three"].entries()) {
      ...
      if (i < 2)
        await mock.executeTool("TaskUpdate", { taskId: String(i + 1), status: "completed" }, ctx);
    }
  ...
  it("keeps the list when auto-clear is off", async () => {
    writeTasksConfig(agentDir, { autoClearCompleted: "never" });
    const { mock, ctx } = await start();
    await runAndFinish(mock, ctx, 1);
  ...
  it("removes the emptied session file and its project-slug directory when the countdown clears the list", async () => {
    const { mock, ctx } = await start();
    await runAndFinish(mock, ctx, 1);
