...
import { serializeAgentFile } from "../../modules/subagents/agent-file-toggle.ts";
import { BUILTIN_TOOL_NAMES } from "../../modules/subagents/agent-types.ts";
...
Agent prompt.`,
    ...
    expect(loadCustomAgents(tmpDir).get("code-review")!.sourcePath).toBe(
      join(tmpDir, ".pi", "agent", "agents", "blubb.md"),
    );
  });
...
Good body.`,
  ...
  it("warns when a skipped file was overriding an agent that stays resolvable", () => {
    ...
    try {
      ...
      expect(message).toContain(
        `Agent "dup" now loads from ${join(globalAgentsDir(), "dup.md")} instead`,
      );
    } finally {
      warn.mockRestore();
    }
  ...
  describe("eject round-trip", () => {
    ...
    it("preserves an explicitly narrowed tool list", () => {
      expect(roundTrip({ builtinToolNames: ["read", "grep"] }).builtinToolNames).toEqual([
        "read",
        "grep",
      ]);
    });
    ...
    it("preserves allowed_subagents in both its list and `all` forms", () => {
      ...
      expect(roundTrip({ allowedSubagents: ["Explore", "Plan"] }).allowedSubagents).toEqual([
        "Explore",
        "Plan",
      ]);
    });
