...
import {
  ...
  type ModelRegistry,
  resolveModel,
} from "../../modules/subagents/model-resolver.ts";
...
describe("describeModel", () => {
  it("gives a short label and a canonical id", () => {
    expect(
      describeModel({ provider: "anthropic", id: "claude-sonnet-4-6", name: "Claude Sonnet 4.6" }),
  ...
  it("falls back to the id when the model has no display name", () => {
    expect(describeModel({ provider: "openai-codex", id: "gpt-5.6-sol" })).toEqual({
