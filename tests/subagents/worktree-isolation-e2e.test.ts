 ...
 * tests/worktree.test.ts drives real git but calls createWorktree/cleanupWorktree
 * directly, with no agent anywhere. tests/agent-manager.test.ts (and -gc, and
 * agent-startup-error) mock ../../modules/subagents/worktree.ts entirely and assert only the gate
...
function agentResultText(session: {
