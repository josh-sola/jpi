...
// The LOAD side (modules/subagents/custom-agents.ts) parses that frontmatter with a real YAML
...
//
// These live in modules/subagents/agent-file-toggle.ts rather than inside the `/agents`
...
import {
  ...
  isEmptyStub,
  locateAgentFile,
} from "../../modules/subagents/agent-file-toggle.ts";
import { parseAgentFrontmatter } from "../../modules/subagents/custom-agents.ts";
...
describe("findAgentFile", () => {
  ...
  describe("locateAgentFile", () => {
    ...
    it("classifies a workspace and a personal source path", () => {
      ...
      expect(
        locateAgentFile("code-reviewer", join(tmpDir, ".agents", "agents", "reviewer.md"), tmpDir),
      ).toMatchObject({ location: "workspace" });
      expect(
        locateAgentFile("code-auditor", join(agentDir, "agents", "auditor.md"), tmpDir),
    ...
    it("falls back when the recorded path has since been deleted", () => {
      ...
      expect(
        locateAgentFile("scout", join(tmpDir, ".agents", "agents", "gone.md"), tmpDir),
      ).toEqual({
        path: join(tmpDir, ".agents", "agents", "scout.md"),
        location: "workspace",
      });
    ...
    it("finds nothing when neither the source path nor the probe resolves", () => {
      expect(
        locateAgentFile("nope", join(tmpDir, ".agents", "agents", "gone.md"), tmpDir),
      ).toBeUndefined();
    });
  });
});
