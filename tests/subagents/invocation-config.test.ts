...
import {
  ...
  resolveJoinMode,
} from "../../modules/subagents/invocation-config.ts";
...
describe("resolveAgentInvocationConfig", () => {
  ...
  it('lets a config isolation of "off" veto a param "worktree"', () => {
    const resolved = resolveAgentInvocationConfig(makeConfig({ isolation: "off" }), {
      isolation: "worktree",
    });
    expect(resolved.isolation).toBeUndefined();
  });
...
describe("resolveAgentInvocationConfig — overridden params (#182)", () => {
  ...
  it("records nothing when only one side named a value", () => {
    ...
    expect(
      resolveAgentInvocationConfig(makeConfig(), { model: "provider/param-model", thinking: "max" })
        .overridden,
    ).toBeUndefined();
  });

  it("records each field independently", () => {
    const resolved = resolveAgentInvocationConfig(makeConfig({ thinking: "low" }), {
      model: "provider/param-model",
      thinking: "max",
    });
