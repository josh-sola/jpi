 ...
 *
 * `node:fs` is mocked wholesale rather than spied on: `modules/subagents/` imports its
...
const { AgentWidget } = await import("../../../modules/subagents/ui/agent-widget.ts");
const { initTheme } = await import("@earendil-works/pi-coding-agent");
const { ConversationViewer } = await import("../../../modules/subagents/ui/conversation-viewer.ts");
const { makeFleet, makeSession, mountViewer, mountWidget } =
  await import("../helpers/perf-fixtures.ts");
