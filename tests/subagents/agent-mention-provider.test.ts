...
import type { AgentRecord, AgentTombstone } from "../../modules/subagents/types.ts";
import { createMentionProvider, mentionRoster } from "../../modules/subagents/ui/agent-mention.ts";
...
describe("agent suggestions", () => {
  ...
  it("offers every agent on a bare @, and still offers files", async () => {
    ...
    const provider = createMentionProvider(
      ...
      () =>
        mentionRoster(
          managerWith(record({ handle: "explore" }), record({ handle: "plan", startedAt: 2000 })),
          [],
        ),
      () => true,
    );
...
describe("composing with another extension's provider", () => {
  ...
  it("is still reachable through a provider registered after us", async () => {
    ...
    expect(
      agentRows(
        await outer.getSuggestions(["@ex"], 0, 3, { signal: new AbortController().signal }),
      ).map((i) => i.value),
    ).toEqual(["@explore"]);
    expect(
      (
        await outer.getSuggestions(["#g"], 0, 2, { signal: new AbortController().signal })
...
describe("named agents and evicted ones", () => {
  ...
  it("still resolves the unlisted type handle of a named agent", () => {
    ...
    const roster = mentionRoster(
      managerWith(
        record({ handle: "explore", alias: "auth-audit", type: "Explore", status: "running" }),
      ),
