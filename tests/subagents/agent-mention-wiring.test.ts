...
vi.mock("../../modules/subagents/mention-clone.ts", () => ({ runMentionClone: vi.fn() }));
...
import { runMentionClone } from "../../modules/subagents/mention-clone.ts";
import {
  ctx,
  flush,
  type Hermetic,
...
describe("mentioning an agent that has never run", () => {
  ...
  it("shows the turn limit it will actually be held to (#181)", async () => {
    ...
    try {
      ...
      const lines = factory({ terminal: { columns: 200 }, requestRender: vi.fn() }, theme)
        .render()
        .join("\n");
...
describe("input that is not a mention", () => {
  ...
  it("the suggestion popup goes quiet too, so @ means only 'attach a file'", async () => {
    ...
    expect(
      await provider.getSuggestions(["@ex"], 0, 3, { signal: new AbortController().signal }),
    ).toBe(files);
  });
});
