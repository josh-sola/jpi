...
vi.mock("../../modules/subagents/output-file.ts", async () => {
  const actual = await vi.importActual<typeof import("../../modules/subagents/output-file.ts")>(
    "../../modules/subagents/output-file.ts",
  );
  return {
    ...actual,
...
import { resumeAgent, runAgent } from "../../modules/subagents/agent-runner.ts";
import { subagentsExtension } from "./helpers/boot-extension.ts";
import {
  ensureOutputFile,
  streamToOutputFile,
  writeInitialEntry,
} from "../../modules/subagents/output-file.ts";
