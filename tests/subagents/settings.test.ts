...
import { NO_FALLBACK } from "../../modules/subagents/agent-types.ts";
import {
  applyAndEmitLoaded,
  applySettings,
  ...
  saveAndEmitChanged,
} from "../../modules/subagents/settings.ts";

/**
 * `tests/core`'s own suite covers the KDL codec (union round-trips,
 ...
 * loaded value and the in-memory setters. `createSubagentsConfig` builds the
 * same `enabled`-wrapped Config the module loader does, so `loaded.value`
 * carries that field too.
 *
...
describe("applySettings", () => {
  ...
  it("calls every setter with the loaded value — every field is always present", () => {
    applySettings(
      fullSettings({ maxConcurrent: 4, graceTurns: 3, defaultJoinMode: "group" }),
      appliers,
    );
