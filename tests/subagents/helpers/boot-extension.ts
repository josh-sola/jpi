 ...
 * REAL extension (modules/subagents/index.ts) and drive its registered tools directly.
 *
...
import setupSubagents from "../../../modules/subagents/index.ts";
import { createSubagentsConfig } from "../../../modules/subagents/settings.ts";

/**
 * Boots the real module the way the `jpi` loader does — load the `subagents`
 * config section, then call `setup(pi, ctx)` — behind the same one-argument
 * shape the pre-module-contract extension factory had, so the existing wiring
 * tests (which call `subagentsExtension(pi)` after seeding settings via
 * `hermeticDir`) don't each need their own config-loading boilerplate.
 */
export async function subagentsExtension(pi: ExtensionAPI): Promise<void> {
  const config = createSubagentsConfig();
  const { value, issues } = await config.load();
  await setupSubagents(pi, { config, value, issues });
}
