...
/**
 * Path to a bare `(pi) => Promise<void>` wrapper around the real subagents
 * module (module-extension.ts) — `additionalExtensionPaths` loads a raw file
 * path as a pi extension directly, so it can't take the `JpiModule` shape
 * `modules/subagents/index.ts` now exports.
 */
const EXTENSION_PATH = fileURLToPath(new URL("./module-extension.ts", import.meta.url));
