...
describe("AgentWidget", () => {
  ...
  it("keeps queued agents on one summary line and finished agents visible", () => {
    ...
    widget.update();
    const lines = factory({ terminal: { columns: 200 }, requestRender: () => {} }, theme)
      .render()
      .join("\n");
...
describe("AgentWidget cost display", () => {
  ...
  function render(showCost: boolean, cost: number): string {
    ...
    widget.setUICtx({
      setStatus: () => {},
      setWidget: (_k: unknown, c: unknown) => {
        factory = c;
      },
    } as any);
    widget.update();
    return factory({ terminal: { columns: 200 }, requestRender: () => {} }, theme)
      .render()
      .join("\n");
  }
  ...
  it("keeps the cost visible after the agent finishes", () => {
    ...
    widget.update();
    const out = factory({ terminal: { columns: 200 }, requestRender: () => {} }, theme)
      .render()
      .join("\n");
...
describe("AgentWidget overflow accounting", () => {
  ...
  it("counts everything it hid — the footer total matches what is missing", () => {
    ...
    const shownRunning =
      counts.running -
      [...Array(counts.running).keys()].filter((i) => !body.includes(`run${i} description`)).length;
    const shownFinished =
      counts.finished -
      [...Array(counts.finished).keys()].filter((i) => !body.includes(`fin${i} description`))
        .length;
  ...
  it("shows the completion line again after a finished agent is resumed", () => {
    ...
    const render = () => {
      widget.update();
      return (
        factory?.({ terminal: { columns: 200 }, requestRender: () => {} }, theme).render() ?? []
      ).join("\n");
    };
