 ...
 *
 *   test/fixtures/.agents/agents/*.md      (pre-configured agent templates)
...
import { getAgentConfig, registerAgents } from "../../modules/subagents/agent-types.ts";
import { loadCustomAgents } from "../../modules/subagents/custom-agents.ts";
import { resolveAgentInvocationConfig } from "../../modules/subagents/invocation-config.ts";
...
const TEMPLATES_DIR = join(FIXTURES_DIR, ".agents", "agents");
...
describe("ext: / tools: scoping — template-driven e2e (real pi-mono, headless)", () => {
  ...
  beforeAll(() => {
    ...
    // Load the templates through the REAL loader (workspace agents come from
    // <cwd>/.agents/agents → FIXTURES_DIR/.agents/agents) and install them in
    // the registry runAgent reads from.
