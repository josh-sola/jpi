...
vi.mock("@earendil-works/pi-coding-agent", () => ({
  createAgentSession,
  // Identity, as pi's own is: `defineTool` exists for the type inference.
...
vi.mock("../../modules/subagents/agent-types.ts", () => ({
...
vi.mock("../../modules/subagents/env.ts", () => ({
  detectEnv: vi.fn(async () => ({ isGitRepo: false, branch: "", platform: "linux" })),
}));

vi.mock("../../modules/subagents/prompts.ts", () => ({
  buildAgentPrompt: vi.fn(() => "system prompt"),
}));

vi.mock("../../modules/subagents/memory.ts", () => ({
  buildMemoryBlock: vi.fn(() => ""),
  buildReadOnlyMemoryBlock: vi.fn(() => ""),
}));

vi.mock("../../modules/subagents/skill-loader.ts", () => ({
  preloadSkills: vi.fn(() => []),
}));

vi.mock("../../modules/subagents/nested-tools.ts", () => ({
...
import {
  ...
  setGraceTurns,
  setRememberAgents,
} from "../../modules/subagents/agent-runner.ts";
...
describe("agent-runner final output capture", () => {
  ...
  it("forwards worktreeBase to the prompt builder, and omits it otherwise", async () => {
    const { buildAgentPrompt } = await import("../../modules/subagents/prompts.ts");
...
import {
  ...
  getConfig,
  getToolNamesForType,
} from "../../modules/subagents/agent-types.ts";
import { createNestedSubagentTools } from "../../modules/subagents/nested-tools.ts";
