...
const { ConversationViewer, RESULT_MAX_CHARS } =
  await import("../../modules/subagents/ui/conversation-viewer.ts");
...
describe("ConversationViewer", () => {
  ...
  describe("render width safety", () => {
      ...
      const messages = [
        { role: "user", content: longLine },
        {
          ...
          content: [
            ...
            { type: "toolCall", id: "t1", name: "tool", arguments: {} },
          ],
        },
    ...
    it("no line exceeds width with embedded ANSI escape codes in content", () => {
      ...
      const messages = [
        {
          role: "assistant",
          content: [{ type: "toolCall", id: "t1", name: "tool", arguments: {} }],
        },
  ...
  describe("Markdown rendering", () => {
    ...
    const toolPair = (text: string, id = "t1") => [
      {
        role: "assistant",
        content: [{ type: "toolCall", id, name: "ctx_execute", arguments: {} }],
        stopReason: "toolUse",
      },
      {
        role: "toolResult",
    ...
    it("never rewrites a generic tool's result, in any markdown mode", () => {
      for (const mode of ["assistant", "all"] as const) {
        const out = strip(
          viewerFor(toolPair("## ctx_execute\n\n3) alpha\n7) beta"), mode).render(80).join("\n"),
        );
    ...
    it("falls back to the literal transcript when a chat component's render throws", () => {
      ...
      const renderSpy = vi
        .spyOn(AssistantMessageComponent.prototype, "render")
        .mockImplementation(() => {
