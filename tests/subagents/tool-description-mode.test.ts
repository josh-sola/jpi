...
import { renderSubagentsKdl, subagentsExtension } from "./helpers/boot-extension.ts";
import { setWorktreeIsolationEnabled } from "../../modules/subagents/worktree.ts";

const EXAMPLE_TEMPLATE = fileURLToPath(
  new URL("./fixtures/agent-tool-description.md", import.meta.url),
);
...
describe("toolDescriptionMode", () => {
  ...
  it("compact mode swaps in the short description with one-line type list", async () => {
    ...
    expect(desc).toContain(
      "- explore: Fast read-only specialist for locating files, tracing code, and answering codebase questions. (Tools:",
    );
  ...
  it("the shipped example template renders byte-identical to the full description", async () => {
    // Guards fixtures/agent-tool-description.md against going stale: it must
