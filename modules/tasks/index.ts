...
export function setupTasks(pi: ExtensionAPI, ctx: ModuleContext<typeof tasksSchema>) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: TASK_CREATE_PROMPT_GUIDELINES,
  ...
  pi.registerTool({
    ...
    execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      // Models sometimes double-escape newlines in tool-call JSON, so this read
      // path tolerates it even though the store round-trip itself is
      // byte-faithful. Cost: a description with a literal backslash-n also
      // gets converted.
