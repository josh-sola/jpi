/**
 * Validates a user-configured key identifier against the grammar
 * `matchesKey` (`@earendil-works/pi-tui`) accepts, since that function itself
 * never rejects a malformed one — it just silently never matches, which
 * would make a typo in `run-background-shortcut` read as "ctrl+b stopped
 * detaching runs".
 */
