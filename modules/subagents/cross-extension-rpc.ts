...
export function registerRpcHandlers(deps: RpcDeps): RpcHandle {
  ...
  const unsubSpawn = handleRpc<{ requestId: string; type: string; prompt: string; options?: any }>(
    ...
    async ({ type, prompt, options }) => {
      ...
      // "openai-codex/gpt-5.5". Resolve it to a real Model instance here so
      // the spawned agent's auth lookup doesn't crash with "No API key found
      // for undefined".
      ...
      if (override != null) {
        ...
        const label =
          typeof override === "string" ? override : `${override.provider}/${override.id}`;
        if (!modelRegistry) {
          throw new Error(
