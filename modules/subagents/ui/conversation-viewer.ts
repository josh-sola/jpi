...
export class ConversationViewer implements Component {
   ...
   */
  private readonly assistantCache = new WeakMap<
    object,
    { component: AssistantMessageComponent; text: string }
  >();
  /**
    ...
    for (const msg of messages) {
      if (msg.role === "user") {
        ...
        for (const content of msg.content) {
          ...
          if (
            !this.appliedToolResults.has(content.id) &&
            (msg.stopReason === "aborted" || msg.stopReason === "error")
          ) {
            const errorMessage =
              msg.stopReason === "aborted" ? "Operation aborted" : msg.errorMessage || "Error";
