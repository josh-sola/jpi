 ...
 * lists this entry. That "declares this entry" check is deliberate: a package
 * installed via `pi.extensions: ["./src/index.ts"]` and named e.g. `pi-subagents`
 * would otherwise misattribute every co-located file under `src/` to that
 * package name.
 */
...
export function installExtensionToolScope(
  ...
  ctx: {
     ...
     *
     * Opt-in nested-delegation tools share EXCLUDED_TOOL_NAMES' names, so they
     * would not otherwise survive a `keep` seeded from `toolNames`.
     */
...
export async function runAgent(
  ...
  // agent's own frontmatter can take back, so `disallowed_tools` is applied
  // HERE rather than at the gates.
