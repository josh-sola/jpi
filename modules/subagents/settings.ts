...
// section of the shared `<agentDir>/jpi.kdl` (core's `Config`). No project
// tier — `.pi/subagents.json` and `<agentDir>/subagents.json` are dead.

import { Config, injectEnabled, j, type WithEnabled } from "../../src/core/index.ts";
...
export const subagentsSchema = j.node({
  fields: {
    ...
    defaultMaxTurns: j
      .number()
      .default(0)
      .describe("default max turns before wrap-up; 0 = unlimited"),
...
/**
 * Builds the exact `Config` the module loader constructs for the `subagents`
 * section (schema wrapped with the loader-injected `enabled` field) — kept
 * here as a single source of truth so tests that need a real Config exercise
 * the same shape production does, without duplicating `injectEnabled` calls.
 */
export function createSubagentsConfig(
  env?: NodeJS.ProcessEnv,
  homeDirectory?: string,
): Config<WithEnabled<SubagentsSchema>> {
  return new Config("subagents", injectEnabled("subagents", subagentsSchema), env, homeDirectory);
}
...
export async function loadSubagentsSettings(
  config: Config<WithEnabled<SubagentsSchema>>,
...
export function persistToastFor(
  ...
  return issues.length === 0
    ...
    : {
        message: `${successMsg} (session only; failed to persist: ${issues.join("; ")})`,
        level: "warning",
      };
}
