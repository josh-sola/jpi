...
| `jpi`                                | One package holding Josh's modules: system prompt, guardian, status, memory, web, exa-web, title, background, subagents, review (`/review`), tasks, scratchpad, btw, schedule, style, and history. Each module has a `jpi.kdl` stanza; `enabled #false` turns it off (restart required). |
